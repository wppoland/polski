...
final class DeliveryTimeService
{
    ...
    {
        $termId = (int) $product->get_meta('_spolszczony_delivery_time_id', true);

        // Per-product override.
        if ($termId > 0) {
            $term = get_term($termId, 'spolszczony_delivery_time');
        ...
        $terms = get_the_terms($product->get_id(), 'spolszczony_delivery_time');
        if (is_array($terms) && ! empty($terms)) {
            return $terms[0]->name;
        }

        // Default fallback.
        $settings = get_option('spolszczony_delivery', []);
        $defaultTermId = is_array($settings) ? (int) ($settings['default_delivery_time'] ?? 0) : 0;

        if ($defaultTermId > 0) {
            $term = get_term($defaultTermId, 'spolszczony_delivery_time');
            if ($term instanceof \WP_Term) {
                return $term->name;
            }
        }
    ...
    public function getDeliveryTimeHtml(\WC_Product $product): string
    {
        ...
        $html = sprintf(
            '<div class="spolszczony-delivery-time"><span class="spolszczony-delivery-time__text">%s</span></div>',
            esc_html($text),
        );
         ...
         */
        return (string) apply_filters('spolszczony/delivery_time/display', $html, $timeText, $product);
    }
}
